...
const DropdownSubMenu = forwardRef<HTMLUListElement, DropdownSubItemProps>((props, ref) => {
    const {
    ...
    const menuitemRef = useUncertainRef<HTMLUListElement>(
        ref as Ref<HTMLUListElement>,
    ) as RefObject<HTMLUListElement | null>
    ...
    return (
        <ul style={style} className="relative" {...rest}>
            <MenuItem
                ref={menuitemRef as RefObject<HTMLElement>}
                disabled={disabled}
                asElement={Component}
                id={menuitemId}
                isActive={active}
