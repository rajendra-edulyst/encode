...
import { Pencil, CalendarCheck2, CalendarPlus, Plus, BadgeCheck, FileText, Trash2 as Trash, Upload, DownloadIcon } from "lucide-react";
...
const ProfileViewFirstRender = () => {
    ...
    const [resumes, setResumes] = useState<{ id: string; url: string; title: string }[]>([])
    ...
    const { data: packageDetails } = useUserPackageDetails(userProfile?.id || 0)
    ...
    const loadResumes = async () => {
        try {
            const response = await listVideoResume();
            if (response && response.resume) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                setResumes(response.resume.map((r: any) => ({
                    id: r.id,
                    url: r.url,
                                        ...
                                        <div className="absolute right-4 top-4">
                                            <div className="flex justify-between items-center space-x-4">
                                                ...
                                                <button onClick={() => setSharePopupOpen(true)}>
                                                    <img src={share} alt="share" width={25} height={27} />
                                                </button>
                                            </div>
                                        </div>
                                                        ...
                                                        userProfile?.persona_stage ?
                                                            (
                                                                <div className='flex items-center gap-2'>
                                                                    ...
                                                                    <span className="text-2xl font-normal font-jacques capitalize text-white"> {userProfile?.persona_stage} </span>
                                                                </div>
                                                            ) : (
                    ...
                    <Dialog open={isPreviewResumeOpen} onOpenChange={setIsPreviewResumeOpen}>
                        <DialogContent className="bg-[#1D1D1D] border-none text-white max-w-4xl h-[90vh] flex flex-col">
                                ...
                                <DialogTitle className="text-2xl font-bold flex justify-between items-center pr-8">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                if (resumes[0]?.id) {
                                                    handleDeleteResume(resumes[0].id);
                                                    setIsPreviewResumeOpen(false);
                                                }
                                            }}
                                        >
                            ...
                            <div className="flex-1 w-full overflow-hidden rounded-xl bg-white mt-4 relative">
                                {resumes[0]?.url ? (
                                    resumes[0].url.toLowerCase().endsWith('.pdf') ? (
                                        <embed
                                            src={`${resumes[0].url}#toolbar=0&navpanes=0&scrollbar=0`}
                                            className="w-full h-full"
                                            type="application/pdf"
                                        />
                                    ) : (
                                        <video controls className="w-full h-full object-contain bg-black">
                                            <source src={resumes[0].url} />
...
export default ProfileViewFirstRender;
