...
import AddResume from '../builder/AddResume';
import PersonalInfo from '../builder/PersonalInfo'

import DescriptionWithClamp from './utils/DescriptionWithClamp';
import { phoneMask } from './utils/phoneMask';
import maskEmail from './utils/maskEmail';

export interface PreviewProps {
    showTypeIn?: "edit" | "view";
    personalInfoHideIn?: boolean;
}

const Preview: React.FC<PreviewProps> = ({ showTypeIn = "view", personalInfoHideIn = true }) => {
    ...
    const [showType, setShowType] = useState<string>(showTypeIn);
    const [personalInfoHide, setPersonalInfoHide] = useState<boolean>(personalInfoHideIn);
            ...
            <div className="flex flex-col gap-4">
                ...
                >
                    ...
                    >
                        ...
                        {/* give a toggel swith btn herew which can swith mode of setShowType to view and setPersonalInfoHide to true */}
                                    ...
                                    <p className="text-base sm:text-lg text-gray-700 mt-1 max-w-xl">
                                        {
                                            Array.isArray(portfolio?.portfolio_profile) && portfolio?.portfolio_profile?.length !== 0 && <div className='flex'>
                                                <p className="text-gray-600">{maskEmail(portfolio?.portfolio_profile[0]?.email, personalInfoHide)}</p>
                                                {portfolio?.portfolio_profile[0]?.phone !== null && (
                                                    <>,<p className="text-gray-600">{phoneMask(portfolio?.portfolio_profile[0]?.phone, personalInfoHide)}</p></>
                                                )}
                                            </div>
                                        }
                ...
                {portfolio?.resume && portfolio?.resume[0]?.url && (
                    ...
                    portfolio={
                        ...
                        portfolio?.portfolio_profile?.length !== 0
                            ? {
                                ...
                                phone: portfolio?.portfolio_profile[0]?.phone != null
                                    ? String(portfolio?.portfolio_profile[0]?.phone)
                                    : undefined
                            }
                            : null
                    }
                    show={showPersonalInfoDialog}
                    onClose={setShowPersonalInfoDialog}
