...
const CreateScormContent = () => {
    ...
    const onSubmit = (data: any) => {
        ...
        formData.append("content_type", "scorm");
        formData.append("program_id", moduleId);
        formData.append("PID_module", id);
        formData.append('content_type_label', 'SCORM');
        ...
        createModuleContent(formData).then(() => {
            toast.success("SCORM content created successfully!");
            reset({
                title: "",
                duration: "",
                points: 0,
    ...
    return (
        <>
            ...
            {id && moduleId && <ContentTypeSelector type={"scorm"} programId={id} moduleId={moduleId} />}
            <Card>
                <CardHeader className='border-b'>
                    <div className="flex items-center gap-2">
                        <BarChart size={35} className="text-primary" />
                        <div>
                            <CardTitle className='text-lg'>SCORM</CardTitle>
                            <span className="text-sm text-gray-500">Add SCORM content for the module</span>
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <Input id="title" placeholder="Enter SCORM title" {...register("title", { required: true, maxLength: 255 })} />
                        ...
                        <div className="space-y-2">
                            ...
                            {(!watch("uploadContent") || watch("uploadContent")?.length === 0) && (
                                <>
                                    ...
                                    <p className="text-xs text-gray-500">Upload SCORM content in ZIP format</p>
                                </>
                            )}
                        ...
                        <div className="flex justify-end">
                            <Button type="submit" variant="default" className="text-white">{content ? loading ? "Updating..." : "Update SCORM" : loading ? "Creating..." : "Create SCORM"}</Button>
                        </div>
                    </form>
                </CardContent>
            </Card>
...
export default memo(CreateScormContent);