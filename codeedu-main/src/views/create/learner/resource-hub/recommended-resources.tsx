...
const Tools = () => {
    ...
    const { data: items = [], isLoading } = useRecommendedResources(params);
                        ...
                        <TabsList className='mb-5 rounded-xl p-0 h-auto bg-gray-200 dark:bg-[#4d4d4d] divide-x divide-gray-300 dark:divide-gray-500 border border-gray-300 dark:border-gray-500 overflow-hidden'>
                            <TabsTrigger value="all" className="capitalize px-4 rounded-none py-2 text-sm font-normal text-white data-[state=active]:font-semibold">All</TabsTrigger>
                            {
                                type.map((typeItem, index) => (
                                    <TabsTrigger key={`tab-${index}`} value={typeItem?.toLowerCase()?.replace(/\s+/g, '-')} className={`capitalize px-4 rounded-none py-2 text-sm font-normal text-white data-[state=active]:font-semibold`}>{typeItem}</TabsTrigger>
                                ))
                            }
                        </TabsList>
                        {
                            ['all', ...type.map(typeItem => typeItem?.toLowerCase()?.replace(/\s+/g, '-'))].map((tabValue, index) => {
                                const tabItems = filteredItems.filter(item =>
                                    tabValue === 'all' ? true : item?.type?.toLowerCase()?.replace(/\s+/g, '-') === tabValue
                                );
                                return (
