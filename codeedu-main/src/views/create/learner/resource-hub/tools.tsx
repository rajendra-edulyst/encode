...
import { Check, ExternalLink, Loader, Plus, Search } from 'lucide-react';
...
const Tools = () => {
    ...
    const formatCategoryName = (cat: string | undefined) => {
        if (!cat) return '';
        if (cat === 'a/v-vault' || cat === 'av-vault' || cat === 'a/v vault') return 'A/V Vault';
        return cat.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    };

    const breadcrumbItems = [
        { label: 'Resource Hub', path: '/explore/resource-hub' },
        { label: formatCategoryName(category) }
    ];

    const subTypes = Array.from(new Set(items.map(item => item.sub_type)));
    const subTypesList = ['all', ...subTypes.map(subtype => subtype?.toLowerCase()?.replace(/\s+/g, '-'))];

    let defaultTab = 'all';
    const rawTab = searchParams.get('tab');
    if (rawTab) {
        if (subTypesList.includes(rawTab)) {
            defaultTab = rawTab;
        } else {
            const fuzzyMatch = subTypesList.find(t => t && rawTab && (t.startsWith(rawTab) || rawTab.startsWith(t)));
            if (fuzzyMatch) {
                defaultTab = fuzzyMatch;
            }
        }
    }
            ...
            <LoadingSection isLoading={isLoading} title='Resource Hub Loading...' />
            {
                (items && !isLoading) && (
                    <Tabs defaultValue={defaultTab}>
                        ...
                        {
                            ['all', ...subTypes.map(subtype => subtype?.toLowerCase()?.replace(/\s+/g, '-'))].map((tabValue, index) => {
                                                ...
                                                {tabItems.map((item, idx) => (
                                                    <Card key={`resource-item-${idx}`} className='relative overflow-hidden hover:shadow-lg transition-shadow rounded-3xl pb-3 flex flex-col'>
                                                        ...
                                                        <CardFooter className='flex justify-end items-center gap-3 mt-auto'>
                                                            ...
                                                            {item.saved === 0 && <div className='bg-primary p-3 rounded-lg h-[96px] w-[126px] flex flex-col justify-center items-center text-center text-black mb-3 cursor-pointer' onClick={() => mapResource(item.id)}>
                                                                {mapResourceMutation.isPending && mapResourceMutation.variables?.[0] === item.id ? <Loader className="w-5 h-5 text-black animate-spin" /> : <Plus />}
                                                                Add to <br />
                                                                Resources
                                                            </div>}
                                                            {item.saved !== 0 && <div className='border text-white border-primary p-3 rounded-lg h-[96px] w-[126px] flex flex-col justify-center items-center text-center text-black mb-3 cursor-pointer' onClick={() => mapResource(item.id)}>
                                                                {mapResourceMutation.isPending && mapResourceMutation.variables?.[0] === item.id ? <Loader className="w-5 h-5 text-black animate-spin" /> : <Check />}
                                                                Already<br />
                                                                Added
                                                            </div>}
                                                        </CardFooter>
