...
function RecommendedCourses() {
    ...
    const params = useMemo(() => {
        ...
        p.append('items', PER_PAGE.toString());
        if (selectedCategory && selectedCategory !== "all") {
            p.append('cat_id', selectedCategory);
        }
        if (searchQuery) {
            p.append('query', searchQuery);
        }
        if (selectedMode && selectedMode !== "all") {
            p.append('mode', selectedMode);
        }
        return p;
    ...
    const courses = useMemo(() => {
        ...
        return [...coursesData].sort((a: Course, b: Course) => a.name.localeCompare(b.name));
    }, [courseData?.data]);
    ...
    // Memoize normalizeDeliveryMode to prevent re-creation on every render
    ...
    const filteredCourses = useMemo(() => {
        ...
        return result;
    }, [courses, selectedSubscription, selectedMode, normalizeDeliveryMode]);
        ...
        <section>
            ...
            <div className="flex flex-col gap-2">
                <div className="md:flex justify-between items-center">
                    <Heading title="Recommended Courses" description={`Recommended our collection of ${pagination?.total} curated courses `} className="mb-3" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-7">
                    {filteredCourses.map((course) => (
                        <Link
                            ...
                            to={`/recommended-courses/details/${course.name?.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'course'}-${course.id}?enroll_disabled=1`}
                            state={{ courseSource: 'recommended' }}
                        >
