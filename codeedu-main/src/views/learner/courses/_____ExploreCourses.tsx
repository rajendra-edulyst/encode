...
function ExploreCourses() {
    ...
    const [inFocusActiveTab, setInFocusActiveTab] = React.useState('All');
    ...
    useEffect(() => {
        setPage(1);
    }, [inFocusActiveTab, selectedCategory, searchQuery, selectedMode]);
    ...
    const params = useMemo(() => {
        ...
        if (inFocusActiveTab === "Free") p.type = "open";
        else if (inFocusActiveTab === "Paid") p.type = "close";
        if (selectedMode && selectedMode !== "all") p.mode = selectedMode;

        return new URLSearchParams(p);
    }, [page, inFocusActiveTab, selectedCategory, searchQuery, selectedMode]);
    ...
    const getActiveFilters = () => {
        ...
        if (selectedMode && selectedMode !== "all") {
            const modeLabel = Object.entries(deliveryModes).find(
                ([key]) => key === selectedMode
            )?.[1] || selectedMode;
            filters.push(`Mode: ${modeLabel}`);
        }
        if (inFocusActiveTab !== "All") {
            filters.push(`Type: ${inFocusActiveTab}`);
        }
        ...
        <section>
            ...
            <div className='flex flex-col gap-2'>
                <div className='flex justify-between items-center'>
                    <Heading title="Explore Courses" description="List of courses you can explore" className='mb-3' />
                    <div className='flex items-center gap-2'>
                        <div className='justify-center items-center grid grid-cols-3 mx-4'>
                            <Button variant={inFocusActiveTab === 'All' ? 'default' : 'outline'} className={`w-full rounded-none rounded-l-md border ${inFocusActiveTab === 'All' && 'text-white'}`} onClick={() => setInFocusActiveTab('All')}>All</Button>
                            <Button variant={inFocusActiveTab === 'Free' ? 'default' : 'outline'} className={`w-full rounded-none border ${inFocusActiveTab === 'Free' && 'text-white'}`} onClick={() => setInFocusActiveTab('Free')}>Free </Button>
                            <Button variant={inFocusActiveTab === 'Paid' ? 'default' : 'outline'} className={`w-full rounded-none rounded-r-md border ${inFocusActiveTab === 'Paid' && 'text-white'}`} onClick={() => setInFocusActiveTab('Paid')}>Paid</Button>
                        </div>
                        <div>
                        ...
                        <div className='relative'>
                            <Input placeholder="Search courses..." className='focus-visible:ring-0 pl-8' value={inputValue} onChange={handleInputChange} />
                            <Search className='absolute top-1/2 left-2 transform -translate-y-1/2 text-gray-500' size={16} />
                        </div>
                        <Button size={'icon'} variant="outline" className='ml-2' onClick={refreshData}><RefreshCcw className={`${isLoading ? 'animate-spin' : ''}`} /></Button>
                    </div>
                </div>
