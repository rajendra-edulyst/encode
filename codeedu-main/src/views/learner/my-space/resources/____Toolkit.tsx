...
const Toolkits = () => {
    ...
    const toolkits = allResources.filter((item) => item.type === "toolkits");




    const tabs = useMemo(() => {
        const uniqueCategories = Array.from(new Set(toolkits.map((item) => item.category)));
    ...
    return (
        <div className='p-6 md:p-10'>
            <div className='flex items-center gap-2'>
                ...
                <h1 className='text-cblack'>Toolkits</h1>

            </div>
            <p className='text-lg mt-3' >
                Next-gen assistants to boost ideas, workflows, and projects.
            </p>
                            ...
                            .map((item) => (
                                <div key={item.name}
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        <span className="text-xs px-1 py-1 rounded-full bg-pink-100 text-pink-600 font-medium">
                                            {item.category}
                                        </span>
                                    </div>
