...
import { TrendingUp, TrendingDown, SquarePen, List, Plus } from 'lucide-react'
import { Card, CardAction, CardContent, CardFooter, CardHeader, CardTitle } from '@/components/ui/card'
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/components/ui/carousel'
import { Link, useNavigate } from 'react-router-dom'
import { useEvents } from '@/hooks/data/collaborate/useEvents'
import { useInstituteMustAttendStats } from '@/hooks/data/collaborate/useJobs'
...
const eventCards = [
    {
        ...
        listingLabel: 'Meetup Listing',
        addLabel: 'Add Meetup',
        listingRoute: '/collaborate/events?category=Community Meetups',
        createRoute: '/collaborate/events/create?category=community meetups',
    },
    {
        title: 'Flagship Event',
        icon: '/img/icons/ticket.png',
        listingLabel: 'Event Listing',
        addLabel: 'Add Event',
        listingRoute: '/collaborate/events?category=Flagship Event',
        createRoute: '/collaborate/events/create?category=flagship event',
    },
    {
        title: 'Career Drive',
        icon: '/img/icons/graduation-cap.png',
        listingLabel: 'Drive Listing',
        addLabel: 'Add Drive',
        listingRoute: '/collaborate/events?category=Drive',
        createRoute: '/collaborate/events/create?category=drive',
    },
    {
        title: 'Immersion Program',
        icon: '/img/icons/streetview.png',
        listingLabel: 'Program Listing',
        addLabel: 'Add Program',
        listingRoute: '/collaborate/events?category=Immersion Programs',
        createRoute: '/collaborate/events/create?category=immersion program',
    },
];

interface MustAttendProps {
    filter?: string
}

const MustAttend: React.FC<MustAttendProps> = ({ filter }) => {
    ...
    const { data: mustAttendStats = [] } = useInstituteMustAttendStats(filter || 'yearly');
    ...
    const params = React.useMemo(() => {
        ...
        const categoryIds = mustAttendStats.map(stat => stat.event_cat_id).filter(id => id).join(',')
        if (categoryIds) {
            p.append('event_category_id', categoryIds)
        }

        return p
    }, [userOrgId, mustAttendStats]);

    // Sample data for events
    const { data: events = [] } = useEvents(params);

    const getStatValue = (name: string) => {
        if (!Array.isArray(mustAttendStats)) return 0;
        return mustAttendStats?.find(item => item.name.toLowerCase().includes(name.toLowerCase()))?.count || 0;
    }
    ...
    return (
        <div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <MetricCard
                    title="Creators Meetup"
                    value={getStatValue('Creators Meetups')}
                />
                <MetricCard
                    title="Flagship Event"
                    value={getStatValue('Flagship Events')}
                />
                <MetricCard
                    title="Career Drive"
                    value={getStatValue('Career Drive')}
                />
                <MetricCard
                    title="Immersion Program"
                    value={getStatValue('Immersion Program')}
                />
            </div>
                                ...
                                <Card key={index} className='bg-[#323232] p-2 flex flex-col h-full'>
                                    <CardContent className='p-2 flex-1 pb-0'>
                                        <div className='bg-[#5A5A5A] w-10 h-10 rounded-lg flex items-center justify-center'>
                                            <img src={card.icon} alt={card.title} className='w-6 h-10 object-contain' />
                                        </div>
