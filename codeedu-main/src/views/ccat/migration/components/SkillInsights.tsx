...
import { fetchJobs, fetchStageSkills, fetchSkillsList, fetchRecommendedJobRoles } from "@/services/learner/OpportunitieService";
...
const SkillInsights: FunctionComponent<SkillInsightsType> = ({
  ...
  const [selectedDomains, setSelectedDomains] = useState<FunctionalDomain[]>([]);
  ...
  const { data: jobListResponse } = useQuery({
    queryKey: isCciStage4 ? ['recommendedJobRoles'] : ['jobListWow', params.toString()],
    queryFn: () => isCciStage4 ? fetchRecommendedJobRoles() : fetchJobs(params),
  });
  ...
  useEffect(() => {
    if (creativeStage) {
      const ascendingSkills = creativeStage.skills.sort((a: any, b: any) => a.updated_at - b.updated_at);
      setSkills(ascendingSkills)
    }
  }, [creativeStage])

  useEffect(() => {
    const apiDomainIds = creativeStage?.domains?.map((d: any) => d.interestarea_id) || [];
    const propDomainIds = domains?.map((d: any) => d.id) || [];
    const alreadySelectedDomains = apiDomainIds.length > 0 ? apiDomainIds : propDomainIds;

    if (alreadySelectedDomains.length > 0) {
      // Create a map or preserve the original order from alreadySelectedDomains
      // Because creativeStage.domains are ordered by updated_at desc (which means latest)
      const filteredDomains = alreadySelectedDomains
        .map((id: any) => functionalDomains.find((fd: any) => fd.id === id))
        .filter(Boolean)
        .slice(0, 2);
  ...
  }, [domains, functionalDomains, creativeStage]);
    ...
    <section className={[styles.skillInsights, className].join(" ")}>
                ...
                <div className={styles.visualSense2}>
                  ...
                  <div className={styles.rectangleParent} style={{ paddingLeft: '20px', paddingRight: '20px', alignItems: 'center', textAlign: 'center', backgroundColor: skills[0]?.color || getSkillColor(skills[0], allSkillsResponse || []) || '#e66b1f' }}>
                    ...
                    <b className={styles.visualSense3} style={{ width: '100%', height: 'auto', fontSize: '18px', lineHeight: '1.2', wordWrap: 'break-word' }}>
                      {skills[0].skill_name || skills[0].skill_skill_name}
                    </b>
                ...
                <div className={styles.problemSolving2}>
                  ...
                  <div className={styles.rectangleGroup} style={{ paddingLeft: '20px', paddingRight: '20px', alignItems: 'center', textAlign: 'center', backgroundColor: skills[1]?.color || getSkillColor(skills[1], allSkillsResponse || []) || '#26dddd' }}>
                    ...
                    <b className={styles.problemSolving3} style={{ width: '100%', height: 'auto', fontSize: '18px', lineHeight: '1.2', wordWrap: 'break-word' }}>
                      {skills[1].skill_name || skills[1].skill_skill_name}
                    </b>
      ...
      <div className={styles.domainprogress}>
        <div className={styles.container28}>
          ...
          <div className={styles.domains}>{selectedDomains.length} Domains</div>
        </div>
        <div className={styles.domainVisuals}>
          {selectedDomains.slice(0, 2).map((domain: any, index: number) => {
            return (
              <Container
                key={index}
      ...
      <div className={styles.jobRoles}>
        ...
        <div className={styles.roleOptions}>
          {jobs.slice(0, 5).map((job: any) => {
            const roleName = job.job_role || job.name;
            return (
              <div
                key={job.id}
              ...
              >
                <div
                  className={styles.creativeThinking}
                  style={{
                    color: '#000',
                    display: '-webkit-box',
                    WebkitLineClamp: 2,
